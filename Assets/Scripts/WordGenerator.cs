...
public class WordGenerator {
    ...
    public WordGenerator(DicionaryTrie dictionary) {
    ...
    public List<string> GenerateWords(List<string> originalWords, string rackWord, int minNumberLetters,
        GenerationOptions options) {
        var allResultWords = new List<string>();
        foreach (var originalWord in originalWords) {
            if (!ExceedsOptions(options, allResultWords)) {
                var resultWords = GenerateWords(originalWord, rackWord, minNumberLetters, options);
                allResultWords.AddRange(resultWords);
            }
        }

        if (allResultWords.Count > 0) {
        ...
        var totalPool = GetLetterCounts(rackWord);
    ...
    private void FindWordsRecursive(
        ...
        int minLength = 1) {
        // 0. STOP CONDITIONS
        if (ExceedsOptions(options, result)) {
            return;
        }
...
//        if (currentWord.Length > 0 && !_dictionary.IsHasWordsStartingWith(currentWord)) {
    ...
    private static bool ExceedsOptions(GenerationOptions options, List<string> resultWords) {
        return resultWords.Count >= options.MaxWordsToCreate ||
               (options.StopAfterFirstWord && resultWords.Count > 0);
    }
